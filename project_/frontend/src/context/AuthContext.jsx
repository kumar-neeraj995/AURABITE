...
export const AuthProvider = ({ children }) => {
    ...
    const checkLoggedIn = async () => {
      ...
      if (storedUser) {
        try {
          ...
          const res = await axios.get(`${import.meta.env.PROD ? '' : (import.meta.env.VITE_API_URL || 'http://localhost:5000')}/api/auth/me`);
  ...
  const login = async (email, password, isOtpLogin = false, role = null) => {
    setLoading(true);
    setError(null);
    try {
      const res = await axios.post(`${import.meta.env.PROD ? '' : (import.meta.env.VITE_API_URL || 'http://localhost:5000')}/api/auth/login`, { email, password, isOtpLogin, role });
      if (res.data.success) {
  ...
  const register = async (username, email, password, role) => {
    ...
    setError(null);
    try {
      const res = await axios.post(`${import.meta.env.PROD ? '' : (import.meta.env.VITE_API_URL || 'http://localhost:5000')}/api/auth/register`, {
        username,
        email,
        password,
        role
