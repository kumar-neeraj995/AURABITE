...
export const AuthProvider = ({ children }) => {
    ...
    const checkLoggedIn = async () => {
      ...
      if (storedUser) {
        try {
          ...
          const res = await axios.get(`${'https://ckksi-14-139-228-10.run.pinggy-free.link'}/api/auth/me`);
  ...
  const login = async (email, password, isOtpLogin = false, role = null) => {
    ...
    setError(null);
    try {
      const res = await axios.post(`${'https://ckksi-14-139-228-10.run.pinggy-free.link'}/api/auth/login`, { email, password, isOtpLogin, role });
      if (res.data.success) {
  ...
  const register = async (username, email, password, role) => {
    ...
    setError(null);
    try {
      const res = await axios.post(`${'https://ckksi-14-139-228-10.run.pinggy-free.link'}/api/auth/register`, {
        username,
        email,
        password,
        role
