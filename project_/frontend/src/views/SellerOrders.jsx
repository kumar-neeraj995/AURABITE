...
const SellerOrders = () => {
  ...
  const fetchSellerOrders = async () => {
    try {
      const res = await axios.get(`${import.meta.env.PROD ? '' : (import.meta.env.VITE_API_URL || 'http://localhost:5000')}/api/orders`);
      if (res.data.success) {
  ...
  const handleStatusChange = async (orderId, newStatus) => {
    try {
      const res = await axios.put(`${import.meta.env.PROD ? '' : (import.meta.env.VITE_API_URL || 'http://localhost:5000')}/api/orders/${orderId}/status`, {
        status: newStatus
      });
