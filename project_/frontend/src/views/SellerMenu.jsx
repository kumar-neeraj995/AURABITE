...
const SellerMenu = () => {
  ...
  const fetchMenu = async () => {
    ...
    try {
      const res = await axios.get(`${import.meta.env.PROD ? '' : (import.meta.env.VITE_API_URL || 'http://localhost:5000')}/api/restaurants/${user.restaurantId}`);
      if (res.data.success) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let res;
      if (isEditing) {
        res = await axios.put(`${import.meta.env.PROD ? '' : (import.meta.env.VITE_API_URL || 'http://localhost:5000')}/api/menu/${editingId}`, payload);
      } else {
        res = await axios.post(`${import.meta.env.PROD ? '' : (import.meta.env.VITE_API_URL || 'http://localhost:5000')}/api/menu`, payload);
      }
  ...
  const handleDelete = async (itemId) => {
    ...
    try {
      const res = await axios.delete(`${import.meta.env.PROD ? '' : (import.meta.env.VITE_API_URL || 'http://localhost:5000')}/api/menu/${itemId}`);
      if (res.data.success) {
        fetchMenu();
      }
    } catch (error) {
