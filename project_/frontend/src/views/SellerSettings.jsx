...
const SellerSettings = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (restaurant) {
        // Edit existing
        res = await axios.put(`${import.meta.env.PROD ? '' : (import.meta.env.VITE_API_URL || 'http://localhost:5000')}/api/restaurants/${restaurant._id}`, payload);
      } else {
        // Create new
        res = await axios.post(`${import.meta.env.PROD ? '' : (import.meta.env.VITE_API_URL || 'http://localhost:5000')}/api/restaurants`, payload);
      }
