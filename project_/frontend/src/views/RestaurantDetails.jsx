...
const RestaurantDetails = () => {
  ...
  const fetchDetails = async () => {
    try {
      const res = await axios.get(`${import.meta.env.PROD ? '' : (import.meta.env.VITE_API_URL || 'http://localhost:5000')}/api/restaurants/${id}`);
      if (res.data.success) {
  ...
  const handleReviewSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(`${import.meta.env.PROD ? '' : (import.meta.env.VITE_API_URL || 'http://localhost:5000')}/api/reviews`, {
  ...
  const handleReviewDelete = async (reviewId) => {
    ...
    try {
      const res = await axios.delete(`${import.meta.env.PROD ? '' : (import.meta.env.VITE_API_URL || 'http://localhost:5000')}/api/reviews/${reviewId}`);
      if (res.data.success) {
        fetchDetails();
      }
    } catch (err) {
  ...
  const handleReviewUpdate = async (e) => {
    ...
    try {
      const res = await axios.put(`${import.meta.env.PROD ? '' : (import.meta.env.VITE_API_URL || 'http://localhost:5000')}/api/reviews/${editingReviewId}`, {
        rating: editRating,
        comment: editComment
      });
